...
PL_PW_dadoscneg = createWriter("PL-PW-virgvers-versicolor15.xt");
...
float SL_SW_peso_bias = linha[linha.length -1], PL_PW_peso_bias=linha2[linha2.length -1];
float bias = 1;
perceptron a,b;
...
a = new perceptron(SL_SW_entrada, SL_SW_peso_entrada, bias, SL_SW_peso_bias, SL_SW_dadoscpos, SL_SW_dadoscneg);
...
b = new perceptron(PL_PW_entrada, PL_PW_peso_entrada, bias, PL_PW_peso_bias, PL_PW_dadoscpos, PL_PW_dadoscneg);
